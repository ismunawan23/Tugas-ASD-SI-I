...
void inputBuku(Buku buku[], int &n) {
    ...
    // Validasi jumlah buku tidak melebihi kapasitas array
    while (n <= 0 || n > 100) {
        cout << "Jumlah buku harus antara 1 hingga 100. Masukkan lagi: ";
        n = validasiInput();
    }
...
// Sequential Search
void sequentialSearch(Buku buku[], int n, int tahun) {
    bool found = false;
    for (int i = 0; i < n; i++) {
...
// Binary Search 
int binarySearch(Buku buku[], int left, int right, int tahun) {
    while (left <= right) {
...
// Catatan Revisi :
// -Program sudah sangat baik dan sudah ada validasi input yang membuatnya lebih user-friendly.

// -Bisa ditingkatkan dengan validasi tambahan, misalnya cek jumlah buku agar tidak melebihi kapasitas array (100).